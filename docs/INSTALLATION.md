...
The installer validates every source file, records existing and previously missing paths, creates a checksum-protected backup, installs the assets, validates Compose and restarts `sogo-mailcow` and `memcached-mailcow`. If a post-change command fails, it attempts an automatic rollback.
